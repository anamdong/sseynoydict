(function(){
  ...
  function convertOne(mc, rimeRomanMap){
    ...
    if (reading === "ri" || reading === "rí" || reading === "rì") {
      reading = reading === "ri" ? "yr" : (reading === "rí" ? "ýr" : "ỳr");
    }
    if (reading === "yi" || reading === "yí" || reading === "yì") {
      reading = reading === "yi" ? "y" : (reading === "yí" ? "ý" : "ỳ");
    }
    if (
      ((mu === "精" && rimeKey === "清三") || ((mu === "從" || mu === "従") && rimeKey === "眞A三")) &&
      reading.normalize("NFD").startsWith("zi")
    ) {
      reading = "j" + reading.slice(1);
    }
  ...
  function searchByReading(dictionary, rawInput){
    ...
    if (!query) return { query, exact: false, groups: [] };

    if (hasToneMarks(query)) {
      const entries = dictionary.readingIndex.get(query) || [];
      return {
        query,
        exact: true,
        groups: entries.length ? [makeReadingGroup(query, entries)] : []
      };
    }

    const folded = foldReading(query);
    const entries = dictionary.foldedReadingIndex.get(folded) || [];
